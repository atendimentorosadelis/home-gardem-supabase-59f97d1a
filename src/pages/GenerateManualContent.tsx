...
function GenerateManualContentPage() {
  ...
  const toggleDesign = (id: string) => {
    setDesignSelected(prev => prev.includes(id) ? [] : [id]);
  };

  const toggleJardim = (id: string) => {
    setJardimSelected(prev => prev.includes(id) ? [] : [id]);
  };
  ...
  const buildJardimTopic = () => {
    const labels = jardimSelected.map(id => JARDIM_TEMAS_GERAIS.find(s => s.id === id)?.label).filter(Boolean);
  ...
  const handleImagesApproved = (coverImage: string | undefined, galleryImages: string[]) => {
    if (article) {
      setArticle({ ...article, coverImage, galleryImages });
    }
    setShowImageApproval(false);
    setShowPreview(true);
  };
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 p-6 md:p-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-foreground">Gerar Conteúdo com IA</h1>
                <p className="text-sm md:text-base text-muted-foreground">Crie artigos completos automaticamente</p>
              </div>
            </div>
          ...
          <>
            <div className="grid gap-4 md:gap-6 lg:grid-cols-3">
              ...
              <Card className="border-border/50 hover:border-primary/30 transition-colors">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-sm">Selecione um cômodo para gerar dicas de design</CardDescription>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Áreas Sociais</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {DESIGN_AREAS_SOCIAIS.map((item) => renderSubnicheCard(item, designSelected, toggleDesign, 'design'))}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Áreas Íntimas</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {DESIGN_AREAS_INTIMAS.map((item) => renderSubnicheCard(item, designSelected, toggleDesign, 'design'))}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Serviço & Externas</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {DESIGN_AREAS_SERVICO.map((item) => renderSubnicheCard(item, designSelected, toggleDesign, 'design'))}
                    </div>
                  </div>
                  <Button className="w-full rounded-xl" onClick={handleGenerateDesign} disabled={isGenerating || designSelected.length === 0}>
              ...
              <Card className="border-border/50 hover:border-primary/30 transition-colors">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-sm">Selecione um tema sobre jardinagem</CardDescription>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    {JARDIM_TEMAS_GERAIS.map((item) => renderSubnicheCard(item, jardimSelected, toggleJardim, 'jardim'))}
                  </div>
                  <Button className="w-full rounded-xl" onClick={handleGenerateJardim} disabled={isGenerating || jardimSelected.length === 0}>
                    {isGenerating ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Gerando...</>) : (<><AIIcon size="sm" className="mr-2" />Gerar Artigo</>)}
                  </Button>
                </CardContent>
              </Card>
              ...
              <Card className="border-border/50 hover:border-primary/30 transition-colors">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-sm">Selecione um estilo arquitetônico</CardDescription>
            ...
            <Card className="border-border/50 hover:border-primary/30 transition-colors">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-sm">Gere artigos temáticos para datas especiais do ano</CardDescription>
              ...
              <CardContent className="space-y-4">
                    ...
                    <CommemorativeDateAlert
                      ...
                      onGenerateArticle={(date) => {
                        ...
                        generateArticle(date.topicSuggestion).then(async (result) => {
                          if (result) {
                            if (requireApproval) { setShowImageApproval(true); } else { setShowPreview(true); }
                            fireConfetti();
                            setArticleSaved(true);
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Todas as Datas</h4>
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isSelected ? "border-primary bg-primary/10 shadow-sm" : "border-border/50 hover:border-primary/50 hover:bg-muted/50"
                          )}
                        >
                          <IconComp className={cn("h-4 w-4 sm:h-5 sm:w-5", isSelected ? "text-primary" : "text-muted-foreground")} />
                          <span className={cn("text-[10px] sm:text-xs font-medium leading-tight line-clamp-2", isSelected ? "text-primary" : "")}>{date.label}</span>
                          <span className="text-[9px] sm:text-[10px] text-muted-foreground">{formattedDate}</span>
                        </div>
                      );
                    })}
                  </div>
                </div>
                <Button className="w-full rounded-xl" onClick={handleGenerateCommemorativeDate} disabled={isGenerating || !commemorativeSelected}>
                  {isGenerating ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Gerando...</>) : (<><AIIcon size="sm" className="mr-2" />Gerar Artigo Comemorativo</>)}
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/50">
              <CardContent className="py-4">
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button variant="outline" onClick={handleRegenerate} disabled={isGenerating} className="rounded-xl">
                    <RefreshCw className="mr-2 h-4 w-4" />Gerar Novo
                  </Button>
                  <Button variant="outline" onClick={handleDiscard} disabled={isGenerating} className="rounded-xl text-destructive border-destructive/50 hover:bg-destructive/10">
                    <X className="mr-2 h-4 w-4" />Excluir
                  </Button>
                  <div className="flex-1" />
                  <Button onClick={handlePublish} disabled={isGenerating} className="rounded-xl shadow-lg shadow-primary/25">
      ...
      <AlertDialog open={showNavigationDialog} onOpenChange={() => {}}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Você tem um artigo gerado que ainda não foi salvo. Se sair agora, perderá todo o conteúdo. Deseja continuar?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={cancelNavigation}>Continuar Editando</AlertDialogCancel>
            <AlertDialogAction onClick={confirmNavigation} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Sair sem Salvar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </DashboardLayout>
